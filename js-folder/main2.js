...
function getvalueInText(i) {
    ...
    if (inputData <= 0) {
        ...
        // }
        console.log(priNumArr[i] );  // 총 매입가격?>
        console.log(avgNummArr[i]);  // 평균단가??
        ...
        console.log(priNumArr[i] );  // 총 매입가격?>
        console.log(avgNummArr[i]);  // 평균단가??
        console.log(plusArr[i]); // 평가손익
        console.log( percentArr[i]); // 수익률
        console.log(allMonArr[i]); // 총 평가금액
        console.log(buyNumArr[i]); //수량
    }
    else {
        alert("돈 없어")
    }
...
function setvalueInText(i) {
    ...
    // 함수로 만들기
    let inputData = document.getElementsByClassName("data")[i].value; // 내가 입력한 매도 개수
    if (inputData <= 0) {
        alert("1이상의 숫자만 넣어주세요")
        // console.log(inputNum.value);
        document.getElementsByClassName("data")[i].value = ""
    ...
    else if (allMonArr[i] >= nowPrice[i].innerHTML * inputData) { // 현재가 * 매도 하는 수량 이 내 평가금액보다 낮을 때만 매도가 가능하지 

        
        // 보유현금
        allMonArr[i] -= nowPrice[i].innerHTML * inputData // 총 평가금액에서 
        allMon[i + 1].innerHTML = allMonArr[i];
        ...
        // 수량 (수량 - 현재수량)
        buyNumArr[i] -= Number(inputData); // 
        text[i + 1].innerHTML = buyNumArr[i] // 수량 자리에 표시
